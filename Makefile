...
BUILD_FLAGS = -ldflags "-X github.com/blockchainworkers/conch/version.GitCommit=`git rev-parse --short=8 HEAD`"
...
draw_deps:
	...
	@goviz -i github.com/blockchainworkers/conch/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
...
rpc-docs:
	...
	godoc2md -template rpc/core/doc_template.txt github.com/blockchainworkers/conch/rpc/core | grep -v -e "pipe.go" -e "routes.go" -e "dev.go" | sed 's,/src/target,https://github.com/blockchainworkers/conch/tree/master/rpc/core,' >> $(DESTINATION)
